...
func CheckGoVersion(goroot string) error {
	...
	if err != nil {
		return fmt.Errorf("GopherJS %s requires a Go 1.17.x distribution, but failed to read its VERSION file: %v", Version, err)
	}
	if !bytes.HasPrefix(v, []byte("go1.17")) {
		return fmt.Errorf("GopherJS %s requires a Go 1.17.x distribution, but found version %s", Version, v)
	}
	return nil
}
