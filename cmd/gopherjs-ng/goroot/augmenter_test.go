...
type SomeType struct{}
// func SomeType.SomeMethod — GopherJS replacement at example.go:1:11
...
func TestAugmenterCollect(t *testing.T) {
	fset := token.NewFileSet()
	f := parse(t, fset, exampleSource)
	...
	if diff := cmp.Diff(want, keys); diff != "" {
		t.Errorf("Augmenter.Collect() symbol keys differ from expected (-want,+got):\n%s", diff)
	}
}

func TestAugmenterPrune(t *testing.T) {
	fset := token.NewFileSet()

	filter := func(names ...string) augmenter {
		f := fset.AddFile("example.go", fset.Base(), 42)
		a := augmenter{Replacements: map[string]token.Pos{}}
		for _, n := range names {
			a.Replacements[n] = f.Pos(10)
		}
		return a
	}
	...
	tests := []struct {
		...
		{
			descr:     "single const",
			augmenter: filter("x.C"),
			original:  "package x; const C int = 1",
			want:      "package x\n// const C — GopherJS replacement at example.go:1:11",
		},
		{
			descr:     "const group",
			augmenter: filter("x.C1"),
			original:  "package x; const (C1 int = 1; C2 int = 2)",
			want: "package x\n" +
				"// const C1 — GopherJS replacement at example.go:1:11\n" +
				"const (C2 int = 2)",
		},
		{
			descr:     "single type",
			augmenter: filter("x.T1"),
			original:  "package x; type T1 int; type T2 bool",
			want: "package x\n" +
				"// type T1 — GopherJS replacement at example.go:1:11\n\n" +
				"type T2 bool",
		},
		{
		...
		{
			descr:     "type group",
			augmenter: filter("x.T1", "x.T2"),
			original:  "package x; type (T1 int; T2 bool)",
			want: "package x\n" +
				"// type T1 — GopherJS replacement at example.go:1:11\n" +
				"// type T2 — GopherJS replacement at example.go:1:11",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.descr, func(t *testing.T) {
			...
			test.augmenter.Prune(fset, f)
			got := reconstruct(t, fset, f)

			if diff := diff.Diff(gofmt(t, test.want), got); diff != "" {
				t.Errorf("Augmenter.Prune() returned diff (-want,+got):\n%s", diff)
			}
		})
	}
}
